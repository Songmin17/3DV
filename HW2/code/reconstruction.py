...
def FindMissingReconstruction(X, track_i):
    ...
    # find the indices of track where value != [-1, -1]
    i1 = np.where(track_i[:, 0] != -1)[0]
    i2 = np.where(track_i[:, 1] != -1)[0]
    track_ind = np.concatenate((i1, i2))
    track_ind = set(track_ind)
    # select only the indices that correspond to valid X. 
    X_ind = np.where(X[:, 0] == -1)[0]
    X_ind = set(X_ind)
    indices = X_ind.intersection(track_ind)
    indices = np.asarray(list(indices), dtype=int)

    # as there may not be any valid indices, we must check if 'indices' is empty. 
    if len(indices) > 0:
        new_point[indices] = 1
