...
def get_sphere_bweights(sph_vs, capsules):
    ...
    cols = []
    # print(f'debug get_sphere_bweights\ncapsules info:\n{len(capsules)}')
    # for elem in capsules:
        # print(elem)
    # TODO: not sure what the TA intended for range(10) + range(12, 22)
    # list concatenation? or list element addition? 
    # NOTE: original: for cps, w in zip(capsules, range(10) + range(12, 22))
    # NOTE: version 1 -> for cps, w in zip(capsules, list(range(10)) + list(range(12, 22)))
    # NOTE: version 2 -> for cps, w in zip(capsules, range(10) + range(12, 22))
    # l1, l2 = list(range(10)), list(range(12, 22))
    # ranges = [l1[i] + l2[i] for i in range(len(l1))] 
